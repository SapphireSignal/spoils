...
def make_scene_overlook() -> Canvas:
    """BACKDROP PITCH — THE OVERLOOK: the district from a hillside at
    night. Lamp constellations down in the valley, one or two lit windows,
    and the wire drawn across the whole of it in broken red. A raider
    stands on the near ridge looking down at the place they have to go
    into. Doubles as the map-select scenic tile.

    Painted the house way: banded cel light, wavy solid seams, no dither
    and no dot noise. Centre-left is kept quiet — the menu buttons sit
    centred over this."""
    rng = random.Random(f"{SEED}:scene:overlook")
    c = Canvas(SCENE_W, SCENE_H)
    HORIZON = 300

    # ---- sky: solid bands, seams wobbled so they never read as stripes
    sky = ["090a14", "10141f", "172038", "1e1d39", "253a5e", "3c5e8b"]
    edges = [0, 70, 130, 190, 240, 275, HORIZON]
    for b in range(len(sky)):
        top, bot = edges[b], edges[b + 1]
        col = C(sky[b])
        phase = rng.uniform(0.0, 6.0)
        for x in range(SCENE_W):
            wob = int(round(3.0 * math.sin(x / 90.0 + phase)
                            + 2.0 * math.sin(x / 37.0 + phase * 1.7)))
            c.rect(x, top + (wob if b else 0), x, bot + wob, col)

    # ---- the moon, low and banded, off to the right of the buttons
    mx, my, mr = 762, 86, 17
    for ring, (rr, name) in enumerate(((mr + 26, "1e1d39"), (mr + 15, "253a5e"),
                                       (mr + 7, "3c5e8b"))):
        col = C(name)
        for y in range(my - rr, my + rr + 1):
            for x in range(mx - rr, mx + rr + 1):
                if (x - mx) ** 2 + (y - my) ** 2 <= rr * rr:
                    c.set(x, y, col)
    for y in range(my - mr, my + mr + 1):
        for x in range(mx - mr, mx + mr + 1):
            d2 = (x - mx) ** 2 + (y - my) ** 2
            if d2 <= mr * mr:
                c.set(x, y, C("c7cfcc") if d2 < (mr - 4) ** 2 else C("a8b5b2"))
    for (sx, sy, sr) in ((mx - 6, my - 3, 4), (mx + 5, my + 4, 3)):   # maria
        for y in range(sy - sr, sy + sr + 1):
            for x in range(sx - sr, sx + sr + 1):
                if (x - sx) ** 2 + (y - sy) ** 2 <= sr * sr:
                    c.set(x, y, C("a8b5b2"))

    # ---- stars: few, deliberate, and only well above the skyline
    for _ in range(34):
        x, y = rng.randrange(8, SCENE_W - 8), rng.randrange(6, 150)
        if abs(x - mx) < 70 and abs(y - my) < 60:
            continue
        c.set(x, y, C("c7cfcc") if rng.random() < 0.3 else C("819796"))

    # ---- far ridge, then the near one: flat silhouettes, no detail
    def ridge(base_y: int, amp: float, period: float, col, seed: float) -> None:
        for x in range(SCENE_W):
            h = int(round(amp * math.sin(x / period + seed)
                          + amp * 0.45 * math.sin(x / (period * 0.37) + seed * 2)))
            c.rect(x, base_y + h, x, HORIZON + 40, col)

    ridge(HORIZON - 22, 9.0, 150.0, C("1e1d39"), 0.6)
    ridge(HORIZON - 8, 6.0, 95.0, C("172038"), 2.3)

    # ---- THE VALLEY FLOOR: the district itself, a skyline of blocks
    ground = C("10141f")
    c.rect(0, HORIZON, SCENE_W - 1, SCENE_H - 1, ground)
    roof_lo, roof_hi = C("202e37"), C("253a5e")
    lit_a, lit_b = C("e8c170"), C("de9e41")
    x = -10
    while x < SCENE_W + 10:
        bw = rng.randint(26, 74)
        bh = rng.randint(16, 54)
        top = HORIZON - bh + rng.randint(-4, 6)
        wall = roof_hi if rng.random() < 0.45 else roof_lo
        c.rect(x, top, x + bw, HORIZON + 26, wall)
        c.hline(x, x + bw, top, C("394a50"))            # lit roof edge
        # a chimney or a vent on some of them
        if rng.random() < 0.4:
            cx0 = x + rng.randint(4, max(5, bw - 8))
            c.rect(cx0, top - rng.randint(6, 13), cx0 + 3, top, wall)
        # windows: sparse, warm, and never a grid
        rows = max(1, bh // 14)
        for r in range(rows):
            wy = top + 6 + r * 12
            wx = x + 4
            while wx < x + bw - 5:
                if rng.random() < 0.16 and wy < HORIZON + 10:
                    c.rect(wx, wy, wx + 2, wy + 3,
                           lit_a if rng.random() < 0.5 else lit_b)
                wx += rng.randint(7, 13)
        x += bw + rng.randint(2, 9)

    # a water tower and a crane, so the skyline has landmarks
    for (tx, th) in ((214, 58), (628, 46)):
        c.rect(tx - 1, HORIZON - th, tx + 1, HORIZON + 8, C("202e37"))
        c.rect(tx - 12, HORIZON - th - 14, tx + 12, HORIZON - th, C("394a50"))
        c.hline(tx - 12, tx + 12, HORIZON - th - 14, C("577277"))
        for legx in (tx - 9, tx + 9):
            c.vline(legx, HORIZON - th, HORIZON + 8, C("202e37"))
    cxx = 848
    c.vline(cxx, HORIZON - 96, HORIZON + 6, C("394a50"))
    c.hline(cxx - 46, cxx + 16, HORIZON - 96, C("394a50"))
    c.vline(cxx - 34, HORIZON - 96, HORIZON - 74, C("202e37"))   # the hook line
    c.rect(cxx - 37, HORIZON - 74, cxx - 31, HORIZON - 70, C("577277"))

    # ---- street lamps: small banded pools strung along the valley road
    for lx in range(60, SCENE_W, rng.randint(74, 96)):
        ly = HORIZON + rng.randint(14, 30)
        if rng.random() < 0.22:
            continue                                    # a dead one
        # a lamp at this distance is a POINT with a short warm bleed under
        # it. The first cut drew a wide ellipse and every lamp read as a
        # glowing eye lying in the street.
        c.vline(lx, ly - 10, ly - 1, C("202e37"))       # the column
        c.set(lx, ly - 11, C("de9e41"))                 # the head
        c.set(lx - 1, ly - 11, C("884b2b"))
        c.set(lx + 1, ly - 11, C("884b2b"))
        for k in range(1, 5):                            # spill on the road
            c.hline(lx - k, lx + k, ly - 10 + k * 2,
                    C("602c2c") if k > 2 else C("884b2b"))

    # ---- THE WIRE: the barricade ring, drawn across the whole valley in
    # broken red. The one thing every raider looks for from up here.
    wire_y = HORIZON + 44
    run = 0
    for x in range(0, SCENE_W):
        drop = int(round(2.5 * math.sin(x / 60.0)))
        if run <= 0 and rng.random() < 0.02:
            run = rng.randint(6, 16)                    # a gap in the line
        if run > 0:
            run -= 1
            continue
        c.set(x, wire_y + drop, C("a53030"))
        c.set(x, wire_y + drop + 1, C("752438"))
        if x % 47 == 0:                                 # posts
            c.vline(x, wire_y + drop - 12, wire_y + drop + 9, C("341c27"))

    # ---- the near hillside: a dark mass the viewer is standing on
    for x in range(SCENE_W):
        h = int(round(16.0 * math.sin(x / 210.0 + 1.1)
                      + 7.0 * math.sin(x / 63.0)))
        c.rect(x, HORIZON + 96 + h, x, SCENE_H - 1, C("090a14"))
        c.hline(x, x, HORIZON + 96 + h, C("172038"))    # rim catches the moon
    # grass tufts along the rim, thinning toward the middle
    for _ in range(150):
        gx = rng.randrange(0, SCENE_W)
        if 360 < gx < 620 and rng.random() < 0.7:
            continue
        gh = int(round(16.0 * math.sin(gx / 210.0 + 1.1)
                       + 7.0 * math.sin(gx / 63.0)))
        gy = HORIZON + 96 + gh
        for k in range(rng.randint(2, 5)):
            c.set(gx + (k // 3), gy - k, C("19332d"))

    # ---- framing pines: hard against the LEFT AND RIGHT EDGES and rooted
    # off the bottom of the frame, so they read as near trees you are
    # standing among. The first cut put them mid-frame and they looked
    # like scenery stranded in the middle of the valley.
    for (px_, ph, w0) in ((22, 250, 30), (86, 196, 23), (938, 236, 28),
                          (876, 180, 21)):
        base_y = SCENE_H + 10
        c.rect(px_ - 3, base_y - ph + 40, px_ + 3, base_y, C("090a14"))
        tiers = 13
        for k in range(tiers):
            t = k / float(tiers - 1)
            yy = int(base_y - 30 - t * (ph - 40))
            half = int(w0 * (1.0 - t * 0.82)) + 3
            c.rect(px_ - half, yy, px_ + half, yy + 9, C("090a14"))
            c.hline(px_ - half, px_ + half, yy, C("151d28"))   # moonlit lip
        c.rect(px_ - 4, base_y - ph - 6, px_ + 4, base_y - ph + 6, C("090a14"))

    # ---- the raider on the near rim, looking down into it
    fx = 300
    gh = int(round(16.0 * math.sin(fx / 210.0 + 1.1) + 7.0 * math.sin(fx / 63.0)))
    fy = HORIZON + 96 + gh
    c.rect(fx - 5, fy - 30, fx + 5, fy - 12, C("172038"))    # coat
    c.rect(fx - 6, fy - 27, fx - 5, fy - 16, C("172038"))    # arms
    c.rect(fx + 5, fy - 27, fx + 6, fy - 16, C("172038"))
    c.rect(fx - 4, fy - 12, fx - 1, fy, C("10141f"))         # legs
    c.rect(fx + 1, fy - 12, fx + 4, fy, C("10141f"))
    c.rect(fx - 4, fy - 39, fx + 4, fy - 30, C("1e1d39"))    # head
    c.hline(fx - 4, fx + 4, fy - 39, C("3c5e8b"))            # moon on the crown
    c.hline(fx - 5, fx + 5, fy - 30, C("253a5e"))            # collar catches it
    return c
