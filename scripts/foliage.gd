...
var _bushes: Array[Dictionary] = []   # {node, sprite, base_x, wig, inside}
var _player: Player
...
func _process(delta: float) -> void:
	...
	for b in _bushes:
		...
		sprite.modulate.a = move_toward(sprite.modulate.a,
			0.55 if inside else 1.0, delta * 4.0)
		if float(b["wig"]) > 0.0:
			b["wig"] = maxf(0.0, float(b["wig"]) - delta)
			var offset := 0.0
			if float(b["wig"]) > 0.15:       # whole pixels only
				offset = 1.0 if fmod(Engine.get_process_frames() / 6.0, 2.0) < 1.0 else -1.0
