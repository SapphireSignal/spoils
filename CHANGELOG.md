...
The door transom is upper storey only, so it hides rather than swaps.
**+26 textures, ZERO extra nodes.**

**Corner posts needed their own low piece after all.** The first cut reused the
one-storey post, on the reasoning that it is exactly the ground band's height —
which is true and still wrong: it is a *different canvas*, and the origins are
**32 px apart, (8,44) against (8,76)**, so swapping to it would displace every
corner. The user saw the result plainly: *"theres some beam going upwards, same
with on each end of the door, so theres 6 beam things still showing up"* —
four corners and the two the door cell adds. `post2_*_low` shares post2's
canvas and origin, and is generated by drawing the post at FULL height (so the
course rolls match) and then lifting the upper band off and re-laying the cap
at the cut.

### Fixed - the walls did not change until you took a step
*"if you go up the stairs, then dont move your character, it only shows the
first floor still"*. The interior update early-outs when your CELL has not
changed, which is a real optimisation — but climbing does not change your cell,
...
still applied as a trim on the master bus rather than by re-cutting individual
sounds: every one is authored under the standing quiet rule, which is precisely
what leaves the headroom to lift the whole mix at once.
